...
## Phase 13: Email OTP Verification — ✅ built 2026-09-25
*New phase, built the same day. A second proof of "this is a real Baze student" alongside Phase 12's ID card — a new first onboarding step, students only.*

Final shape: students verify control of their registered `@bazeuniversity.edu.ng` mailbox with a 4-digit emailed code before reaching the avatar/language steps — onboarding is now 3 steps. **Originally built as SMS via Termii, switched to email the same day**: Nigerian SMS needs a registered sender ID backed by a CAC certificate and a website with a Nigerian address, which a student project doesn't have (Twilio and Africa's Talking have the same requirement; Termii's WhatsApp OTP needs a WhatsApp Business account, a dedicated SIM and a verifiable business). Termii also rejected the shared `N-Alert` sender I'd assumed needed no approval. The SMS code was removed, not left dormant — see git commit `732c261` if SMS becomes viable.

- [✅] The dispatcher (`renderOnboardingPage()`) gates on the persisted `user.emailVerified` flag, not a step counter, so a student who already verified skips the step on their next visit; avatar/language steps still restart on refresh as before.
- [✅] The code is always emailed to the account's own address (`user.email`), never a client-supplied one — no input needed on the step. `utils/mailer.js#sendOtpEmail` reuses the Gmail transporter but is **awaited and throws** (unlike the fire-and-forget review emails), since onboarding can't proceed without it.
- [✅] Protections: 60s resend cooldown (only stamped after a successful send), 10-minute expiry, 5 wrong guesses per code; code stored in plain text on `User.emailOtpCode`.
- [✅] `User` fields `emailVerified` (exposed) + backend-only `emailOtpCode`/`emailOtpExpiresAt`/`emailOtpAttempts`/`emailOtpSentAt`. Endpoints `POST /api/users/email/send-otp` and `/verify-otp` (`controllers/emailVerificationController.js`, mounted in `routes/userRoutes.js`).
- [✅] Frontend `renderOnboardingEmailStep()`; the OTP field updates state on `input` without `render()` (same reason as the ID-card file input). Existing steps relabeled "Step 2 of 3" and "Step 3 of 3".
- [✅] Verified end to end: wrong code, correct code, cooldown, and a real email sent through Gmail then entered in headless Chromium through to the avatar step.
- [ ] Needs `GMAIL_USER`/`GMAIL_APP_PASSWORD` on Render; OTP emails may land in spam.
- [ ] No live client-side countdown on the resend cooldown — the server's 429 message is shown instead.
## Phase 10: UI Polish
